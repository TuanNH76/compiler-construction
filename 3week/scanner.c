...
Token* getToken(void) {
  ...
  case CHAR_TIMES:
    ...
    cn = colNo;
    readChar();
    if (currentChar == '*') { // Kiểm tra ký tự tiếp theo
      readChar();
      return makeToken(SB_POWER, ln, cn); // Trả về token SB_POWER cho `**`
    } else {
      return makeToken(SB_TIMES, ln, cn); // Trả về token SB_TIMES cho `*`
    }
  case CHAR_SLASH:
