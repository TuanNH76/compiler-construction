...
void compileRepeatSt(void) {
  ...
  eat(KW_REPEAT);
  compileStatements(); // Phân tích các lệnh trong REPEAT
  eat(KW_UNTIL);
  compileCondition();  // Phân tích điều kiện UNTIL
...
void compileTerm2(void) {
  ...
  if (lookAhead->tokenType == SB_POWER) {
    eat(SB_POWER);        // Nhận diện `**`
    compileFactor();      // Phân tích phần tử tiếp theo
    compileTerm2();       // Đệ quy để xử lý tiếp các phép toán lũy thừa
  } else if (lookAhead->tokenType == SB_TIMES || lookAhead->tokenType == SB_SLASH) {
    eat(lookAhead->tokenType);
    compileFactor();
    compileTerm2();
  }
